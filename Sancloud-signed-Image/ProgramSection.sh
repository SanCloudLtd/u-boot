...
if [[ PLAIN_ACCESS -ne 0 ]]; then
	sshpass -p "$PASSWORD" ssh -o StrictHostKeyChecking=no -o LogLevel=ERROR -o UserKnownHostsFile=/dev/null "debian@${IP_ADDRESS}" "echo \"$PASSWORD\" | sudo -S -p ''   env -C ~/winbond-lib/build/  ./TESTAPP -w -p -f $FILE -s ${SECTION} -D ${DIE} $FSIZE"
else
	if [ -n "$KEY" ]; then
